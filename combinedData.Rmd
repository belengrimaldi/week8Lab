...
northeast_words <- filter(region_words, region == "North East", tf_idf > 5.0e-05)

ne_imp_bing <- northeast_words %>%
  inner_join(get_sentiments("bing"))

ggplot(ne_imp_bing, 
    aes(x = sentiment, fill=sentiment)) + 
    ggtitle("North East Relatively Important Words Sentiment Range") +
    geom_bar() +
    theme_minimal() +
    labs(x = 'Sentiment', y = 'Count') +
    scale_color_manual(values = c('red', 'green'))

midwest_words <- filter(region_words, region == "Midwest", tf_idf > 5.0e-05)

mw_imp_bing <- midwest_words %>%
  inner_join(get_sentiments("bing"))

ggplot(mw_imp_bing, 
    aes(x = sentiment, fill=sentiment)) + 
    ggtitle("Midwest Relatively Important Words Sentiment Range") +
    geom_bar() +
    theme_minimal() +
    labs(x = 'Sentiment', y = 'Count') +
    scale_color_manual(values = c('red', 'green'))
...
# Word Cloud by Region... could use editing
bing_regions %>% 
  group_by(region) %>% 
  slice_sample(n = 25) %>% 
  ungroup() %>% 
  ...
  geom_text_wordcloud() +
  facet_wrap(~region) +
  labs(x = NULL, y = NULL, title='Words and Sentiment by Region') +
  theme_minimal()
